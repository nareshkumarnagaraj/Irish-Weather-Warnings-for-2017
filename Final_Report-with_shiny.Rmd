...
\
Likewise, we can check the wind direction across all the stations:
\
```{r, message=FALSE, warning=FALSE}
Station_Names <- o %>%
  filter(station %in% unique(station)) %>% group_by(station)
ui <- fluidPage(
  
  # Application title
  titlePanel("Wind Directions Analysis"),
  
  sidebarLayout(
    sidebarPanel(
      selectInput("select", h3("Station"), 
                  choices = o$station)),
...
server <- function(input, output) {
  output$regPlot <- renderPlot({
    x2<-o %>% select(station, hour, wddir) %>% filter(station==input$select)
    plot<-ggplot(x2, aes(x=hour, y=wddir))+geom_line(color = "Blue",linetype = "dotdash",lwd=1) 
    plot+ scale_y_continuous(limits = c(0, 360), breaks = seq(0, 360, by = 90))+
  xlab("Hour")+ylab("Wind Angles(in Degree)")
  })
